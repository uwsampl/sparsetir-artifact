        ...
        with T.block("bsrmm0"):
            ...
            for j in T.serial(J_indptr[vi + 1] - J_indptr[vi]):
                with T.block("bsrmm1"):
                    ...
                    C[vi, vbi, vf] = (
                        C[vi, vbi, vf]
                        + A[vi, vj, vbi, vbj] * B[J_indices[vi, vj], vbj, vf]
                    )
...
def hyper_gnn(
    ...
    J_indptr = T.match_sparse_buffer(indptr, [I], dtype="int32", extra_storage=1)
    J_indices = T.match_sparse_buffer(indices, [I, J_dense], dtype="int32")
    I_T_indptr = T.match_sparse_buffer(
        indptr_T, [J_detach], dtype="int32", extra_storage=1
    )
            ...
            for j in T.serial(J_indptr[vi + 1] - J_indptr[vi]):
                with T.block("hyper_gnn1"):
                    ...
                    with T.init():
                        Y[vi, vf] = T.float32(0)
                    for i_t in T.serial(
                        I_T_indptr[J_indices[vi, vj] + 1]
                        - I_T_indptr[J_indices[vi, vj]]
                    ):
...
def ellmm(
    ...
    for i, j, bi, bj, f in T.grid(nb, col, blk, blk, feat_size):
        with T.block("ellmm0"):
            ...
            T.reads(
                A[vi, vj, vbi, vbj], B[J_indices[vi, vj], vbj, vf], J_indices[vi, vj]
            )
            T.writes(C[vi, vbi, vf])
            T.block_attr({"sparse": True})
            with T.init():
...
def square_sum(
    ...
    K_indptr = T.match_sparse_buffer(
        indptr_k, [I, J_dense], dtype="int32", extra_storage=1
    )
...
def square_sum_two_K(
    ...
    J_indptr = T.match_sparse_buffer(indptr_j, [I], dtype="int32", extra_storage=1)
    J_indices = T.match_sparse_buffer(indices_j, [I, J_dense], dtype="int32")
    K0_indptr = T.match_sparse_buffer(
        indptr_k0, [I, J_dense], dtype="int32", extra_storage=1
    )
    K0_indices = T.match_sparse_buffer(indices_k0, [I, J, K0_dense], dtype="int32")
    K1_indptr = T.match_sparse_buffer(
        indptr_k1, [I, J_dense], dtype="int32", extra_storage=1
    )
            ...
            for j in T.serial(J_indptr[vi + 1] - J_indptr[vi]):
                with T.block("binary_search_block_0_1"):
                    ...
                    T.reads(
                        ...
                        K0_indices[
                            vi, vj, 0 : K0_indptr[vi, vj + 1] - K0_indptr[vi, vj]
                        ],
                    )
                        ...
                        with T.block("binary_search_block_0_2"):
                            ...
                            T.reads(
                                K0_indices[
                                    vi,
                                    vj,
                                    0 : K0_indptr[vi, vj + 1] - K0_indptr[vi, vj],
                                ]
                            )
                            T.writes(mid_0[vi, vj, vk])
                            T.block_attr({"preprocess": True, "sparse": True})
                            low = T.alloc_buffer(
                                [1], dtype="int32", strides=[1], scope="local"
                            )
                            high = T.alloc_buffer(
                                [1], dtype="int32", strides=[1], scope="local"
                            )
                            ...
                            while low[0] < high[0]:
                                if (
                                    K0_indices[vi, vj, mid_0[vi, vj, vk]]
                                    < K1_indices[vi, vj, vk]
                                ):
        ...
        with T.block("square_sum0"):
            ...
            for j in T.serial(J_indptr[vi + 1] - J_indptr[vi]):
                with T.block("square_sum1"):
                    vj = T.axis.reduce(N1, j)
                    T.reads(
                        K1_indptr[vi, vj : vj + 2], A[vi, vj, 0:N2], mid_0[vi, vj, 0:N2]
                    )
                    T.writes(B[vi])
                    T.block_attr({"sparse": True})
                    with T.init():
...
def fused_reduction_4d_2d(
    ...
    L_indptr = T.match_sparse_buffer(
        indptr_l, [I, J, K], dtype="int32", extra_storage=1
    )
    # body
    # with T.block("root")
    T.assume_buffer_domain(J_indptr, [0, nnz_j])
    ...
    for j in T.serial(nnz_j):
        with T.block("reduction_4d_2d0"):
            ...
            T.reads(
                K_indptr[vi, vj : vj + 2],
                L_indptr[vi, vj, 0:32769],
                X[vi, vj, 0:32768, 0:32768],
            )
            T.writes(Y[vi, vj])
            T.block_attr({"sparse": True})
            for k in T.serial(K_indptr[vi, vj + 1] - K_indptr[vi, vj]):
        ...
        with T.block("rgcn-forward0"):
            ...
            for j, fi in T.grid(J_indptr[vi + 1] - J_indptr[vi], feat_size):
                with T.block("rgcn-forward1"):
                    ...
                    Y[vi, vfo] = (
                        Y[vi, vfo] + W[E[vi, vj], vfo, vfi] * X[J_indices[vi, vj], vfi]
                    )
...
def rgcn_hetero_forward(
    ...
    J_indptr = T.match_sparse_buffer(
        indptr_j, [R, I_dense], dtype="int32", extra_storage=1
    )
        ...
        with T.block("rgcn-hetero-forward0"):
            ...
            for i in T.serial(I_indptr[vr + 1] - I_indptr[vr]):
                with T.block("rgcn-hetero-forward1"):
                    ...
                    T.block_attr({"sparse": True})
                    for j, fi in T.grid(
                        J_indptr[vr, vi + 1] - J_indptr[vr, vi], feat_size
                    ):
...
def csr2bsr(
    ...
    J_bsr = T.sparse_variable(
        I_bsr, (n_out, nnz_out), (indptr_out, indices_out), "int32"
    )
    ...
    J_bsr_indptr = T.match_sparse_buffer(
        indptr_out, [I_bsr], dtype="int32", extra_storage=1
    )
    J_bsr_indices = T.match_sparse_buffer(
        indices_out, [I_bsr, J_bsr_dense], dtype="int32"
    )
    # body
    # with T.block("root")
    mid_0 = T.alloc_sparse_buffer([I, J], dtype="int32", extra_storage=0)
            ...
            for j in T.serial(J_indptr[vi + 1] - J_indptr[vi]):
                with T.block("binary_search_block_0_1"):
                    ...
                    T.reads(
                        J_bsr_indices[
                            vi // blk_size,
                            0 : J_bsr_indptr[vi // blk_size + 1]
                            - J_bsr_indptr[vi // blk_size],
                        ]
                    )
                    T.writes(mid_0[vi, vj])
                    T.block_attr({"preprocess": True, "sparse": True})
                    ...
                    low[0] = 0
                    high[0] = (
                        J_bsr_indptr[vi // blk_size + 1] - J_bsr_indptr[vi // blk_size]
                    )
                    mid_0[vi, vj] = low[0] + (high[0] - low[0]) // 2
                    while low[0] < high[0]:
                        if (
