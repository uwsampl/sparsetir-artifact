...
def bench_bsrmm(bsr_mat: Any, x: th.Tensor):
    ...
    mod = tvm.IRModule.from_expr(bsrmm.specialize(
        {v_mb: mb, v_nb: nb, v_nnzb: nnzb, v_blk: block_size, v_feat_size: feat_size}
    ))
...
def bench_cublas(W: th.Tensor, X: th.Tensor):
    with th.no_grad():
        W = W.half().to(0)
        X = X.to(0)
        timer = benchmark.Timer(stmt="a @ b",
                                globals={
                                    'a': W,
                                    'b': X
                                })
...
if __name__ == "__main__":
    tokenizer = AutoTokenizer.from_pretrained("vuiseng9/bert-base-squadv1-block-pruning-hybrid")

    model = AutoModelForQuestionAnswering.from_pretrained("vuiseng9/bert-base-squadv1-block-pruning-hybrid")
    ...
    for name, param in model.named_parameters():
        if name.endswith(
            ...
            bsr_weight = sp.bsr_matrix(param.detach().numpy(), shape=param.shape, blocksize=(32, 32))
